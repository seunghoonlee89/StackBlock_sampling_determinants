...
void CIcoeff_from_mps_TRIE()
{
  ...
  if(dmrginp.spinAdapted())
  {
    ...
    if (dmrginp.targetState() == -1){
        ...
        mps1->build(dmrginp.targetState());
    }
    zeromps = mps1;
  }
  else{
  ...
  pout <<"|0> norm"<<zeromps->get_norm()<<endl;
//  /*
//  {
...
//      }
//      double temp =sampledmps->get_norm()*overlap*overlap/(e*coeff*coeff);
