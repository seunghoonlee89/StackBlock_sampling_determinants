 ...
 //************
  ...
  if (dmrginp.calc_type() == COMPRESS)
  {
	...
	targetState = dmrginp.targetState();
      ...
      if (mpigetrank()==0) {
  // generate stateinfo for truncated, untruncated from rotation matrix
	Sweep::InitializeStateInfo(sweepParams, direction, baseState);
	Sweep::InitializeStateInfo(sweepParams, !direction, baseState);
  // generate wavefunction with one specific wavefunction 
  ...
  else if (dmrginp.calc_type() == COMPRESS_MPSINIT)
  {
    //lsh
    bool direction; int restartsize;
    //sweepParams.restorestate(direction, restartsize);
  ...
  // Initialize TargetState (MPS Initialized)
  // generate stateinfo for truncated, untruncated from rotation matrix
  ...
  // generate wavefunction with one specific wavefunction 
    ...
    //this genblock is required to generate all the nontranspose operators
    dmrginp.setimplicitTranspose() = false;
    SweepGenblock::do_one(sweepParams, false, false, false, restartsize, targetState, baseState);
